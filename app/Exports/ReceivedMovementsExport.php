...
class ReceivedMovementsExport implements FromCollection, WithHeadings, WithMapping, WithStyles
{
    protected $movements;
    ...
    public function collection()
    {
        ...
        foreach ($this->movements as $movement) {
            // 1. صف الحركة الرئيسي للوارد
            $rows->push([
                'type'          => 'movement',
                'movement_id'   => $movement->id,
                'branch_from'   => optional($movement->branchfrom)->name ?? '---', // الفرع المحول منه
                'branch_to'     => optional($movement->branchto)->name ?? '---',   // الفرع المحول إليه (المستقبل)
    ...
    public function headings(): array
    {
        return [
            'رقم الحركة',
            'الفرع المحول منه (المصدر)',
            'الفرع المحول إليه (المستقبل)',
            'المستخدم المسؤول',
            'الحالة',
            'تاريخ الإنشاء',
            'بيان المنتج / الحركة',
