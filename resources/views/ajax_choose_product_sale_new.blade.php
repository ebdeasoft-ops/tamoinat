            ...
            <tr id="<?php echo $product['id']; ?>">
                ...
                <td id="tableData">
                    ...
                    @if($product->All_QUENTITY<=0)
                     <button style="padding: 6px 12px" type="button" id="btn" name="btn" class="btn btn-danger" data-dismiss="modal" onclick="chooseProduct('{{$product->id}}','{{$product->barcode}}','{{$product->name}}','{{$product->cost}}','{{$product->price}}','{{$product->Product_Location}}','{{$product->All_QUENTITY}}','{{$currentrow}}')">{{__('home.Add')}}</button>

                        @else

                        <button style="padding: 6px 12px" type="button" id="btn" name="btn" class="btn btn-success" data-dismiss="modal" onclick="chooseProduct('{{$product->id}}','{{$product->barcode}}','{{$product->name}}','{{$product->cost}}','{{$product->price}}','{{$product->Product_Location}}','{{$product->All_QUENTITY}}','{{$currentrow}}')">{{__('home.Add')}}</button>
