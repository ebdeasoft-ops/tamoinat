    ...
    <table class="table table-hover align-middle text-center mb-0" style="vertical-align: middle;">
        <thead class="bg-light text-dark">
            <tr>
                ...
                <th class="py-3 font-weight-bold">{{ __('From Branch') }} <br><small class="text-muted">من فرع</small></th>
        ...
        <tbody>
            @forelse($receivedMovements as $index => $movement)
                <tr class="transition-hover">
                    <td class="font-weight-bold text-muted">{{ $receivedMovements->firstItem() + $index }}</td>
                    <td>
                        <span class="badge badge-pill badge-success px-3 py-2 font-weight-bold" style="font-size: 0.85rem;">
                            #{{ $movement->id }}
                        </span>
                    </td>
                    <td>
                        <span class="text-dark font-weight-bold">
                            {{ $movement->branchFrom->name ?? __('Not Specified') }}
                        </span>
                    </td>
                    <td>
                    ...
                    <td>
                        <button type="button" class="btn btn-sm btn-outline-success px-3 rounded-pill" data-toggle="modal" data-target="#itemsModal{{ $movement->id }}">
                ...
                <tr>
                    <td colspan="9" class="text-center py-5">
                        <div class="text-muted">
                            ...
                            <p class="mb-0 font-weight-bold">{{ __('home.no_received_movements_found') }}</p>
                        </div>
                    </td>
                </tr>
            @endforelse
...
<div class="card-footer bg-white border-0 py-3 d-flex justify-content-between align-items-center flex-wrap ajax-pagination">
    <div class="text-muted small mb-2 mb-md-0">
        عرض من {{ $receivedMovements->firstItem() ?? 0 }} إلى {{ $receivedMovements->lastItem() ?? 0 }} من أصل {{ $receivedMovements->total() }} حركة
    </div>
    <div>
        {!! $receivedMovements->appends(request()->query())->links() !!}
    </div>
</div>
