...
<div class="main-parent">
    ...
    $count=App\Models\invoices::where('branchs_id',Auth()->user()->branchs_id)->where('save',1)->where('sent_to_zatca',1)->where('status',0)->orderby('id','desc')->count();
    
    ?>
    <!-- breadcrumb -->
    <div class="breadcrumb-header justify-content-between parent-heading">
        <div class="my-auto">
            <h4 class="content-title mb-0 my-auto">{{ __('home.previousSales_sended_Invoices') }}</h4>
        </div>
        
          <div class="my-auto">
            <h4 class="content-title mb-0 my-auto">{{ __('home.previousSales_sended_Invoices') }} : ({{$count}})</h4>
        </div>
    </div>
    <!-- breadcrumb -->
    @endsection
...
<script>
  ...
  $.ajax({
                url: " {{URL::to('getAllinvicesajax_send_zatca')}}",
                type: "GET",
                dataType: "html",
                success: function(products) {
                    $("#ajax_responce_allinvoicesDiv").html(products);
